...
-- explore = press (Caret `ha` Newline) (Event `ha` switch `ha` Focused `ho__'ha` Scope `hv` at @(Mode Unit))
--  `lo'ys'la` First `ha` press (Glyph `ha` Letter `ha` Lower `ha` J) (Event `ha` shift `ha` Down `ho__'ha` Scope `hv` at @(Scrolling List Task))
--  `lo'ys'la` First `ha` press (Glyph `ha` Letter `ha` Lower `ha` K) (Event `ha` shift `ha` Lift `ho__'ha` Scope `hv` at @(Scrolling List Task))
--  `lo'ys'la` First `ha` press (Glyph `ha` Letter `ha` Upper `ha` T) (Event `ha` switch `ha` TODO `ho__'ha` Scope `hv` at @(Scrolling List Task) `ho_` Scope `hv` focus @Only `ho_'he` Scope `hv` at @Mark)
--  `lo'ys'la` First `ha` press (Glyph `ha` Letter `ha` Upper `ha` D) (Event `ha` switch `ha` DONE `ho__'ha` Scope `hv` at @(Scrolling List Task) `ho_` Scope `hv` focus @Only `ho_'he` Scope `hv` at @Mark)
...
draft = intro @(State `T'I` Mode `P` Scrolling List Task `JNT` World) Unit
 ...
 `yok____` New `ha` State `ha___` Event `ha` (shift `ho'ho'yui` Unit) `ho__'ha` Scope `hv` at @(Scrolling List Task)
   `la___` New `ha` State `ha___` Event `ha` (switch `ho'ho'yui` Unit) `ho__'ha` Scope `hv` at @(Scrolling List Task) `ho_` Scope `hv` focus `ho_'he` Scope `hv` at @Mark
   `la___` New `ha` State `ha___` Event `ha` (switch `ho'ho'yui` Unit) `ho__'ha` Scope `hv` at @Mode
  `la____` New `ha` State `ha___` Event `ha` ((\_ -> pop) `ho'ho'yui` Unit) `ho__'ha` Scope `hv` at @(Scrolling List Task) `ho_` Scope `hv` focus `ho_'he` Scope `hv` at @Title
   `la___` New `ha` State `ha___` Event `ha` (push `ha` Glyph `ho'ho'yui` Unit) `ho__'ha` Scope `hv` at @(Scrolling List Task) `ho_` Scope `hv` focus `ho_'he` Scope `hv` at @Title
   `la___` New `ha` State `ha___` Event `ha` (switch `ho'ho'yui` Unit) `ho__'ha` Scope `hv` at @Mode
 `yok____` Again `ha` Once

main = draft `he'he'hv` (Explore `lo` start `li` Unit)

-- (State `T'I` X `JNT` World)
-- (State `T'I` Y `JNT` World)

-- (State `T'I` X `S` Y `JNT` World)
