...
type Command = Shifter List `S` Mark

pattern Move e = This e
pattern Mark e = That e
...
match = exact `ha` Glyph `ha` Letter `ha` Lower `hv'he` J `ho'yo` (Move `ha` Down)
 ...
 `lo'ys'la` exact `ha` Glyph `ha` Letter `ha` Upper `hv'he` T `ho'yo` (Mark `ha` TODO) `ho` Check
 `lo'ys'la` exact `ha` Glyph `ha` Letter `ha` Upper `hv'he` D `ho'yo` (Mark `ha` DONE) `ho` Check
